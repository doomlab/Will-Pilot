...
```{r exp1_source_descriptives, results = 'asis', echo = FALSE, message = F}
...
tableprint[ , 1] = print(unique(final$Source))
...
apa_table(as.data.frame(tableprint),
          note = "Readability statistics were calculated using the Flesch-Kincaid Grade Level readability formula.",
          caption = "Experiment 1 - Descriptive Statistics by Source", 
          col.names = c("Source", "$M_Valence", "$SD_Valence$", "Number of Articles", "Total Words", "$M_Tokens$", "$SD_Tokens$", "$M_Types$", "SD_Types", "$M_Readability$", "$SD_Readability$"),
          format = "latex",
          escape = F,
          align = c("l", rep("c", 6))
          )
...
HUFFPO_kav = read.csv("exp2_data/HUFFPO_kav.csv", stringsAsFactors = F, fileEncoding="UTF-8") #not ok fix
...
#read it in
NYtimes_gs = read.csv("exp2_data/NYtimes_gs.csv", stringsAsFactors = F, fileEncoding = "UTF-8") #not ok 
...
tableprint[ , 4] = tapply(final_kav$X, final$Source, length)
...
tableprint[ , 4] = tapply(final_gs$X, final$Source, length)
