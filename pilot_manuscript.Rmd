...
```{r exp1sourcedescriptives, results = 'asis', echo = FALSE, message = F}
...
Table 3 displays the descriptive statistics for sources' writing on the Kavanaugh confirmation. The sources are similar in some aspects yet different in others. Valence appears to be slightly positive across all sources. The standard deviations seem to indicate little to no presence of a difference in valence. *The New York Times* published the greatest number of articles as well as total words. *Breitbart* featured the lowest number of articles, and *NPR* the lowest number of total words from all articles. Per indvidual article, however, *Breitbart* appears to feature the highest average number of words as well as unique words. Once again the standard deviations call into question any apparent differences between sources. Finally, *Fox News* appears to be the most readable source on average, while *The New York Times* is the least readable. This might be attributable to the greater number of tokens in the average *New York Times* article compared to *Fox News*. As before, the standard deviations for readability hamper one's ability to draw solid conclusions regarding differences in the sources' use of language. Table 4 displays descriptive statistics for articles about the partial government shutdown. 

```{r exp2sourcedescriptiveskav, results = 'asis', echo = FALSE, message = F}
#Kav
...
```{r exp2sourcedescriptivesgs, results = 'asis', echo = FALSE, message = F}
#Kav
#GS
