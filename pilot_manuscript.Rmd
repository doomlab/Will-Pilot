...
#Reading the HTML code from the website - headlines
webpage <- read_html(url)
headline_data = html_nodes(webpage,'a')
...
````{r NPR, eval = F, include = F}
urlslist_final = c()

##loop through the page number at the end until done with ~1000 / 20 = 50
for (i in 1:2){ ##change this to 50
  url2 = paste0('https://www.npr.org/search?query=kavanaugh&page=', i)
  webpage2 = read_html(url2)
  headline_data2 = html_nodes(webpage2,'.title')
  ...
  #add them to an overall
  urlslist_final = c(urlslist_final, urlslist2)
  
  Sys.sleep(runif(1, 1, 5))
}

##start a data frame
NPRDF = matrix(NA, nrow = length(urlslist_final), ncol = 3)
colnames(NPRDF) = c("Source", "Url", "Text")
NPRDF = as.data.frame(NPRDF)
...
````{r Slate, eval = F, include = F}
url3 = 'https://slate.com/search?q=kavanaugh&via=homepage_nav_search'
webpage3 = read_html(url3)
headline_data3 = html_nodes(webpage3,'.gs-title')
...
for (i in 1:length(urlslist3)){
  ...
  ##pull the specific nodes
  headline_data3 = html_nodes(webpage3,'.slate-paragraph') 
  ...
  SlateDF$Text[i] = paste(text_data3, collapse = "")
} ##end for loop
```
