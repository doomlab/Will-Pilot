...
#read it in
NYtimes_kav = read.csv("exp2_data/NYtimes_kav.csv")
NPR_kav = read.csv("exp2_data/NPR_kav.csv")
SLATE_kav = read.csv("exp2_data/SLATE_kav.csv")
HUFFPO_kav = read.csv("exp2_data/HUFFPO_kav.csv")
POLITICO_kav = read.csv("exp2_data/POLITICO_kav.csv")
FOX_kav = read.csv("exp2_data/FOX_kav.csv")
BREITBART_kav = read.csv("exp2_data/BREITBART_kav.csv")
RUSH_kav = read.csv("exp2_data/RUSH_kav.csv")
BLAZE_kav = read.csv("exp2_data/BLAZE_kav.csv")
HANNITY_kav = read.csv("exp2_data/HANNITY_kav.csv")

#merge it together
overalldata_kav = rbind(NYtimes_kav, NPR_kav)
overalldata_kav = rbind(overalldata_kav, SLATE_kav)
overalldata_kav = rbind(overalldata_kav, HUFFPO_kav)
overalldata_kav = rbind(overalldata_kav, POLITICO_kav)
overalldata_kav = rbind(overalldata_kav, FOX_kav)
overalldata_kav = rbind(overalldata_kav, BREITBART_kav)
overalldata_kav = rbind(overalldata_kav, RUSH_kav)
overalldata_kav = rbind(overalldata_kav, BLAZE_kav)
overalldata_kav = rbind(overalldata_kav, HANNITY_kav)

write.csv(overalldata_kav, "exp2_data/overalldata_kav.csv", row.names = F)

##pull in the data
master_kav = read.csv("exp2_data/overalldata_kav.csv", stringsAsFactors = F)
...
##all data
final_kav = subset(nodis, URLS == FALSE)

tapply(final_kav$wordcount, final_kav$Source, mean)
tapply(final_kav$wordcount, final_kav$Source, sum)

write.csv(final_kav, "exp2_data/finaldata_kav.csv")
```

```{r data_clean_gs, eval = T, echo = F}
##combine all gs data into one dataset
#read it in
NYtimes_gs = read.csv("exp2_data/NYtimes_gs.csv")
NPR_gs = read.csv("exp2_data/NPR_gs.csv")
SLATE_gs = read.csv("exp2_data/SLATE_gs.csv")
HUFFPO_gs = read.csv("exp2_data/HUFFPO_gs.csv")
POLITICO_gs = read.csv("exp2_data/POLITICO_gs.csv")
FOX_gs = read.csv("exp2_data/FOX_gs.csv")
BREITBART_gs = read.csv("exp2_data/BREITBART_gs.csv")
RUSH_gs = read.csv("exp2_data/RUSH_gs.csv")
BLAZE_gs = read.csv("exp2_data/BLAZE_gs.csv")
HANNITY_gs = read.csv("exp2_data/HANNITY_gs.csv")

#merge it together
overalldata_gs = rbind(NYtimes_gs, NPR_gs)
overalldata_gs = rbind(overalldata_gs, SLATE_gs)
overalldata_gs = rbind(overalldata_gs, HUFFPO_gs)
overalldata_gs = rbind(overalldata_gs, POLITICO_gs)
overalldata_gs = rbind(overalldata_gs, FOX_gs)
overalldata_gs = rbind(overalldata_gs, BREITBART_gs)
overalldata_gs = rbind(overalldata_gs, RUSH_gs)
overalldata_gs = rbind(overalldata_gs, BLAZE_gs)
overalldata_gs = rbind(overalldata_gs, HANNITY_gs)

write.csv(overalldata_gs, "exp2_data/overalldata_gs.csv", row.names = F)

##pull in the data
master_gs = read.csv("exp2_data/overalldata_gs.csv", stringsAsFactors = F)
...
##all data
final_gs = subset(nodis, URLS == FALSE)

tapply(final_gs$wordcount, final_gs$Source, mean)
tapply(final_gs$wordcount, final_gs$Source, sum)

write.csv(final_gs, "exp2_data/finaldata_gs.csv")
```

```{r stem_text_both, eval = T, echo = F}
##stem Kav text
final_kav = read.csv("exp2_data/finaldata_kav.csv", stringsAsFactors = F)
#final = na.omit(final) #one weird NA line
...
##stem GS text
final_gs = read.csv("exp1_data/finaldata_gs.csv", stringsAsFactors = F)

#process the Text column (write a loop) - save processed text as a separate column
for(i in 1:nrow(final_gs)) {
  
  final_gs$edited[i] = preprocess(fina_gs$Text[i], #one value at a time
           case = "lower", 
           remove.punct = TRUE,
           remove.numbers = FALSE, 
           fix.spacing = TRUE)
...
```
```{r stem_warriner_exp2, eval = T, echo = F}
##open the Warriner data for later
warriner = read.csv("exp1_data/BRM-emot-submit.csv", stringsAsFactors = F) #want v.mean.sum
warriner$V.Mean.Sum = scale(warriner$V.Mean.Sum)
...
##report descriptive statistics of the foundations
#kav
mean(harm_final_data_kav$valence)
sd(harm_final_data_kav$valence)
mean(fair_final_data_kav$valence)
sd(fair_final_data_kav$valence)
mean(ingroup_final_data_kav$valence)
sd(ingroup_final_data_kav$valence)
mean(authority_final_data_kav$valence)
sd(authority_final_data_kav$valence)
mean(purity_final_data_kav$valence)
sd(purity_final_data_kav$valence)
#gs
mean(harm_final_data_gs$valence)
sd(harm_final_data_gs$valence)
mean(fair_final_data_gs$valence)
sd(fair_final_data_gs$valence)
mean(ingroup_final_data_gs$valence)
sd(ingroup_final_data_gs$valence)
mean(authority_final_data_gs$valence)
sd(authority_final_data_gs$valence)
mean(purity_final_data_gs$valence)
sd(purity_final_data_gs$valence)
#note that a big problem is the valence of the words 
```
...
## Results
```{r exp2 source descriptives_both, results = 'asis', echo = FALSE, message = F}
#Kav
...
```{r exp2-table_kav, results = 'asis', echo = F}
#MFD, mean, sd, mean, sd, t, p, d
...
#calculate d
#harm
harm_d_kav = d.ind.t(m1 = mean(harm_final_temp_kav$MFDweight[harm_final_temp$lean == "Conservative"], na.rm = T), 
...
```{r exp2-table_gs, results = 'asis', echo = F}
#MFD, mean, sd, mean, sd, t, p, d
...
#calculate d
#harm
harm_d_gs = d.ind.t(m1 = mean(harm_final_temp_gs$MFDweight[harm_final_temp$lean == "Conservative"], na.rm = T), 
