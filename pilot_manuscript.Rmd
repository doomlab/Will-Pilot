...
ingroup_all = read.csv("iexp1_data/ngroup_words_total.csv", stringsAsFactors = F)
...
#Get the final dataset split into the 4 sources
final$Source = as.factor(final$Source)
NYtimesDesc1 = subset(final, Source == "NY Times") #number of obs. = number of articles for that source; NYT = 406
NPRDesc1 = subset(final, Source == "NPR") #NPR = 695 articles
FoxDesc1 = subset(final, Source == "Fox News") #Fox = 503 articles
BreitbartDesc1 = subset(final, Source == "Breitbart") #Breitbart = 1,437 articles

##Add valence
#NY Times
NYTimes_final_desc = data.frame("URL" = NA, "Source" = NA, "Var1" = NA, "Freq" = NA, "wordcount" = NA, "Text" = NA)

for (i in 1:nrow(NYtimesDesc1)) {
##first make a table of a response, unlisting everything
temp = as.data.frame(table(unlist(strsplit(NYtimesDesc1$stemmed[i], " "))))
...
#add to the dataset
temp2$URL = NYtimesDesc1$Url[i]
temp2$Source = NYtimesDesc1$Source[i]
temp2$wordcount = NYtimesDesc1$wordcount[i]
temp2$Text = NYtimesDesc1$Text[i]

#bind with original
NYTimes_final_desc = rbind(NYTimes_final_desc, temp2)
}

NYTimes_final_desc$valence = vlookup(NYTimes_final_desc$Var1, #index in original data
                                  warriner, #where's the stuff you want to put in
                                  "V.Mean.Sum", #what column is the stuff you want
                                  "Word2") #what's the matching index

#NPR
NPR_final_desc = data.frame("URL" = NA, "Source" = NA, "Var1" = NA, "Freq" = NA, "wordcount" = NA, "Text" = NA)

for (i in 1:nrow(NPRDesc1)) {
##first make a table of a response, unlisting everything
temp = as.data.frame(table(unlist(strsplit(NPRDesc1$stemmed[i], " "))))
...
#add to the dataset
temp2$URL = NPRDesc1$Url[i]
temp2$Source = NPRDesc1$Source[i]
temp2$wordcount = NPRDesc1$wordcount[i]
temp2$Text = NPRDesc1$Text[i]

#bind with original
NPR_final_desc = rbind(NPR_final_desc, temp2)
}

NPR_final_desc$valence = vlookup(NPR_final_desc$Var1, #index in original data
                                  warriner, #where's the stuff you want to put in
                                  "V.Mean.Sum", #what column is the stuff you want
                                  "Word2") #what's the matching index

#Fox News
Fox_final_desc = data.frame("URL" = NA, "Source" = NA, "Var1" = NA, "Freq" = NA, "wordcount" = NA, "Text" = NA)

for (i in 1:nrow(FoxDesc1)) {
##first make a table of a response, unlisting everything
temp = as.data.frame(table(unlist(strsplit(FoxDesc1$stemmed[i], " "))))
...
#add to the dataset
temp2$URL = FoxDesc1$Url[i]
temp2$Source = FoxDesc1$Source[i]
temp2$wordcount = FoxDesc1$wordcount[i]
temp2$Text = FoxDesc1$Text[i]

#bind with original
Fox_final_desc = rbind(Fox_final_desc, temp2)
}

Fox_final_desc$valence = vlookup(Fox_final_desc$Var1, #index in original data
                                  warriner, #where's the stuff you want to put in
                                  "V.Mean.Sum", #what column is the stuff you want
                                  "Word2") #what's the matching index

#Breitbart
Breitbart_final_desc = data.frame("URL" = NA, "Source" = NA, "Var1" = NA, "Freq" = NA, "wordcount" = NA, "Text" = NA)

for (i in 1:nrow(BreitbartDesc1)) {
##first make a table of a response, unlisting everything
temp = as.data.frame(table(unlist(strsplit(BreitbartDesc1$stemmed[i], " "))))
...
#add to the dataset
temp2$URL = BreitbartDesc1$Url[i]
temp2$Source = BreitbartDesc1$Source[i]
temp2$wordcount = BreitbartDesc1$wordcount[i]
temp2$Text = BreitbartDesc1$Text[i]

#bind with original
Breitbart_final_desc = rbind(Breitbart_final_desc, temp2)
}

Breitbart_final_desc$valence = vlookup(Breitbart_final_desc$Var1, #index in original data
                                  warriner, #where's the stuff you want to put in
                                  "V.Mean.Sum", #what column is the stuff you want
                                  "Word2") #what's the matching index

##Eliminate NAs
NYTimes_final_desc = na.omit(NYTimes_final_desc)
NPR_final_desc = na.omit(NPR_final_desc)
Fox_final_desc = na.omit(Fox_final_desc)
Breitbart_final_desc = na.omit(Breitbart_final_desc)

#Calculate mean valence by source
NYTimes_valence_mean = mean(NYTimes_final_desc$valence)
NPR_valence_mean = mean(NPR_final_desc$valence)
Fox_valence_mean = mean(Fox_final_desc$valence)
Breitbart_valence_mean = mean(Breitbart_final_desc$valence)


```
overall valence for each source

types/tokens ratios, etc. for each source also number of articles 

table - columns are types (unique words), tokens (total words), readability statistics, number of articles, total number of words, valence
...
````{r HuffPo, eval = F, include = F}
url4 = 'https://search.huffingtonpost.com/search?utf8=%E2%9C%93&p=kavanaugh&fr=huffpost_desktop-s'
webpage4 = read_html(url4)
headline_data4 = html_nodes(webpage4,'.fw-b')
...
##start a data frame
HuffpoDF = matrix(NA, nrow = length(urlslist4), ncol = 3)
...
for (i in 1:length(urlslist4)){
  ...
  ##pull the specific nodes
  headline_data4 = html_nodes(webpage4,'p') 
  
  ##pull the text
  text_data4 = html_text(headline_data4)
  
  ##save the data
  HuffpoDF$Source[i] = "Huffington Post"
  HuffpoDF$Url[i] = urlslist4[i]
  HuffpoDF$Text[i] = paste(text_data4, collapse = "")
} ##end for loop
```

````{r Politico, eval = F, include = F}
url5 = 'https://www.politico.com/search?q=kavanaugh&s=newest'
webpage5 = read_html(url5)
headline_data5 = html_nodes(webpage5,'.summary a')
...
##start a data frame
PoliticoDF = matrix(NA, nrow = length(urlslist5), ncol = 3)
...
for (i in 1:length(urlslist5)){
  ...
  ##pull the specific nodes
  headline_data5 = html_nodes(webpage5,'.story-text > p') 
  ...
  PoliticoDF$Url[i] = urlslist5[i]
  PoliticoDF$Text[i] = paste(text_data5, collapse = "")
} ##end for loop
```

````{r Fox News, eval = F, include = F}
url6 = 'https://www.foxnews.com/search-results/search?q=kavanaugh&ss=fn&min_date=2018-09-13&max_date=2018-10-11&start=0'
webpage6 = read_html(url6)
headline_data6 = html_nodes(webpage6, 'h3 .ng-binding')

attr_data6 = html_attrs(headline_data6) 
attr_data6

urlslist6 = unlist(attr_data6)
urlslist6 = urlslist6[grep("http|.html", urlslist6)]

##start a data frame
FoxDF = matrix(NA, nrow = length(urlslist6), ncol = 3)
colnames(FoxDF) = c("Source", "Url", "Text")
FoxDF = as.data.frame(FoxDF)

##for loops
for (i in 1:length(urlslist6)){
  
  ##read in the URL
  webpage6 <- read_html(urlslist6[i])
  
  ##pull the specific nodes
  headline_data6 = html_nodes(webpage6,'p+ p , .fn-video+ p , .twitter+ p , .speakable') 
  
  ##pull the text
  text_data6 = html_text(headline_data6)
  
  ##save the data
  FoxDF$Source[i] = "Fox News"
  FoxDF$Url[i] = urlslist6[i]
  FoxDF$Text[i] = paste(text_data6, collapse = "")
} ##end for loop
```

````{r Breitbart, eval = F, include = F}
url7 = 'https://www.breitbart.com/search/?s=kavanaugh#gsc.tab=0&gsc.q=kavanaugh&gsc.page=1'
webpage7 = read_html(url7)
headline_data7 = html_nodes(webpage7, '.gs-title')

attr_data7 = html_attrs(headline_data7) 
attr_data7

urlslist7 = unlist(attr_data7)
urlslist7 = urlslist7[grep(urlslist7)]

##start a data frame
BreitbartDF = matrix(NA, nrow = length(urlslist7), ncol = 3)
colnames(BreitbartDF) = c("Source", "Url", "Text")
BreitbartDF = as.data.frame(BreitbartDF)

##for loops
for (i in 1:length(urlslist7)){
  
  ##read in the URL
  webpage7 <- read_html(urlslist7[i])
  
  ##pull the specific nodes
  headline_data7 = html_nodes(webpage7,'p , h2') 
  
  ##pull the text
  text_data7 = html_text(headline_data7)
  
  ##save the data
  BreitbartDF$Source[i] = "Breitbart"
  BreitbartDF$Url[i] = urlslist7[i]
  BreitbartDF$Text[i] = paste(text_data7, collapse = "")
} ##end for loop
```

````{r Rush Limbaugh, eval = F, include = F}
url8 = 'https://www.rushlimbaugh.com/?s=kavanaugh&is_v=1'
webpage8 = read_html(url8)
#headline_data8 = html_nodes(webpage8, 'a')    trying something different
headline_data8 = html_nodes(webpage8, '.ais-hits--content a')

attr_data8 = html_attrs(headline_data8) 
attr_data8

urlslist8 = unlist(attr_data8)
urlslist8 = urlslist8[grep(urlslist8)]

##start a data frame
RushDF = matrix(NA, nrow = length(urlslist8), ncol = 3)
colnames(RushDF) = c("Source", "Url", "Text")
RushDF = as.data.frame(RushDF)

##for loops
for (i in 1:length(urlslist8)){
  
  ##read in the URL
  webpage8 <- read_html(urlslist8[i])
  
  ##pull the specific nodes
  headline_data8 = html_nodes(webpage8,'p') 
  
  ##pull the text
  text_data8 = html_text(headline_data8)
  
  ##save the data
  RushDF$Source[i] = "Rush Limbaugh"
  RushDF$Url[i] = urlslist8[i]
  RushDF$Text[i] = paste(text_data8, collapse = "")
} ##end for loop
```


````{r Blaze, eval = F, include = F}
...
````{r Sean Hannity, eval = F, include = F}
url10 = 'https://www.hannity.com/'
webpage10 = read_html(url10)
headline_data10 = html_nodes(webpage10, '.gs-title')

attr_data10 = html_attrs(headline_data10) 
attr_data10

urlslist10 = unlist(attr_data10)
urlslist10 = urlslist10[grep(urlslist10)]

##start a data frame
SeanDF = matrix(NA, nrow = length(urlslist10), ncol = 3)
colnames(SeanDF) = c("Source", "Url", "Text")
SeanDF = as.data.frame(SeanDF)

##for loops
for (i in 1:length(urlslist10)){
  
  ##read in the URL
  webpage10 <- read_html(urlslist10[i])
  
  ##pull the specific nodes
  headline_data10 = html_nodes(webpage10,'p') 
  
  ##pull the text
  text_data10 = html_text(headline_data10)
  
  ##save the data
  SeanDF$Source[i] = "Sean Hannity"
  SeanDF$Url[i] = urlslist10[i]
  SeanDF$Text[i] = paste(text_data10, collapse = "")
} ##end for loop
```
