...
library(papaja) #R says this package doesn't exist in current version :/
library(MuMIn)
...
```{r model-table, results='asis', include=FALSE}
tableprint = matrix(NA, nrow = 10, ncol = 10)


##stuff you had before
##three models
table = matrix(NA, nrow = 5, ncol = 10)

##ARE ALL OF THESE COLUMNS NEEDED?? ALSO, ORDER?
colnames(table) = c("Model", "Degrees of Freedom", "AIC", "BIC", 
                    "Log Likelihood", "R2m", "R2c", "Test", "L Ratio", "p-value")

##stick in the information you need
table[1, ] = c("Model 1", 2, 43610.91, 
               43627.55, -21803.46, "", "",
               "", "", "")

table[2, ] = c("Model 2", 3, 43611.04, 
               43636.01, -21802.52, 0.000, 0.004,
               "Model 1 vs Model 2", 1.871644, 0.173)

table[3, ] = c("Model 3", 4, 43604.33, 
               43637.62, -21798.17, 0.0003, 0.0037,
               "Model 2 vs. Model 3", 8.703020, 0.0032)

table[4, ] = c("Model 3.1", 13, 37913.11, 
               38021.30, -18943.55, 0.17, 0.19,
               "Model 3 vs. Model 3.1", 5709.23, "< .001")

table[5, ] = c("Model 3.2", 22, 37904.00, 
              38087.09, -18930.00, 0.17, 0.19,
               "Model 3.1 vs. Model 3.2", 27.11, 0.001)

```

# Results 

To analyze if news sources adhered to differences in word use based on their target audience, we utilized a multilevel model (MLM) to analyze the data. MLM is a regression technique that allows one to control for the repeated measurement and nest structured of the data, which creates correlated error [@CITE_gelman]. First, an intercept only model was created using percent text overall as the dependent variable. Using the *nlme* library in *R* [@CITE_nlme], a model with the news source as the random intercept was analyzed to determine if it was necessary to control for correlated error. Table @\ref(tab:model-table) includes the model statistics for all models. 





#stopped here to ask questions 

Next, a predictor model with political lean predicting percent of MFD words, a model adding MFD category (i.e. "harm-positive," "harm-negative", etc.) as a predictor, and a model with the interaction of political lean and MFD category predicting percent were run. Finally, a series of 10 models were run with political lean predicting percent split into the 10 MFD categories.
...
r.squaredGLMM(model3) #gives you two, report both
...
##overall percent - no random intercept
model4 = lme(percent ~ 1,
             data = long_mlm, 
             method = "ML", 
             na.action = "na.omit",
             random = ~1|partno) #How to get rid of intercept??
summary(model4)
r.squaredGLMM(model4)

##overall percent - random intercept by source
model4.1 = lme(percent ~ 1,
             data = long_mlm, 
             method = "ML", 
             na.action = "na.omit",
             random = ~1|Source)
...
model5.2 = lme(percent ~ lean,
             data = long_mlm[long_mlm$moraltype == "hperlo", ], 
             method = "ML", 
             na.action = "na.omit",
             random = ~1|Source)
summary(model5.2)
r.squaredGLMM(model5.2)

model5.3 = lme(percent ~ lean,
             data = long_mlm[long_mlm$moraltype == "fperhi", ], 
             method = "ML", 
             na.action = "na.omit",
             random = ~1|Source)
summary(model5.3)
r.squaredGLMM(model5.3)

model5.4 = lme(percent ~ lean,
             data = long_mlm[long_mlm$moraltype == "fperlo", ], 
             method = "ML", 
             na.action = "na.omit",
             random = ~1|Source)
...
model5.5 = lme(percent ~ lean,
             data = long_mlm[long_mlm$moraltype == "iperhi", ], 
             method = "ML", 
             na.action = "na.omit",
             random = ~1|Source)
summary(model5.5)
r.squaredGLMM(model5.5)

model5.6 = lme(percent ~ lean,
             data = long_mlm[long_mlm$moraltype == "iperlo", ], 
             method = "ML", 
             na.action = "na.omit",
             random = ~1|Source)
summary(model5.6)
r.squaredGLMM(model5.6)

model5.7 = lme(percent ~ lean,
             data = long_mlm[long_mlm$moraltype == "aperhi", ], 
             method = "ML", 
             na.action = "na.omit",
             random = ~1|Source)
summary(model5.7)
r.squaredGLMM(model5.7)

model5.8 = lme(percent ~ lean,
             data = long_mlm[long_mlm$moraltype == "aperlo", ], 
             method = "ML", 
             na.action = "na.omit",
             random = ~1|Source)
summary(model5.8)
r.squaredGLMM(model5.8)

model5.9 = lme(percent ~ lean,
             data = long_mlm[long_mlm$moraltype == "pperhi", ], 
             method = "ML", 
             na.action = "na.omit",
             random = ~1|Source)
summary(model5.9)
r.squaredGLMM(model5.9)

model5.10 = lme(percent ~ lean,
             data = long_mlm[long_mlm$moraltype == "pperlo", ], 
             method = "ML", 
             na.action = "na.omit",
             random = ~1|Source)
summary(model5.10)
r.squaredGLMM(model5.10)
...
Any more screening/data prep here?

Political lean of the news organization was used to predict the percent of words matching the Moral Foundations Dictionary. According to an analysis of variance (ANOVA) between the intercept-only model (Model 1) and the random intercept-only model (Model 2, *R2m* = 0.00, *R2c* = 0.004), there were no statistically significant differences (*F* = *?*, *p* = 0.17). There was a significant difference between the random intercept-only model and the predictor model (Model 3, *R2m* = 0.0003, *R2c* = 0.0037), *F* = *?*, *p* = .003).  We found a significant difference between the two political leans (*b* = -0.018, *SE* = 0.006, *t*(3039) = -2.95, *p* = 0.003). Next, we added Moral Foundation Type as a predictor of percent to the third model (Model 3.1, *R2m* = 0.17, *R2c* = 0.19). The ANOVA revealed a significant difference between Models 3 and 3.1, *F* = *?*, *p* < .001)

#Significance for, like, all of the little moral types. Put in table?

Finally, we analyzed the interaction between political lean and Moral Foundation type as a predictor of percent (Model 3.2, *R2m* = 0.17, *R2c* = 0.19). Using ANOVA, we found a significant difference between Models 3.1 and 3.2, *F* = *?*, *p* = .001).

#Again, lots of stuff here. Needs to be reported? Put in table?

Comparisons between the models can be found in Table 1. 
# Discussion
How it turned out
