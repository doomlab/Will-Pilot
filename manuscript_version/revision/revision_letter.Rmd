...
The section in the literature review describing the Frimer study has been expanded like shown below.
...
Given no page limits for the manuscript, we have opted to include this detail for transparency and openness purposes within the manuscript. Linguistic analysis heavily relies on the pre-processing steps taken on the data, and informing readers on these actions can help contextualize interpretation. We have streamlined in method sections when it was repetitive refering back to previous studies. 
...
We added an example table of the weighting calculation and rephrased this section completely for clarity. 
...
In response to the question about analytic choices, we have added information about a MTMM study (study 1), as well as the exact same statistics using the mean percentages for comparison (study 2-3). The results remain the same. 
