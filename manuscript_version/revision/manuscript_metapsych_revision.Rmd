...
A complete example of the survey can be found online at https://osf.io/5uqfn. First, participants were given a description of associative memory as the relation between words that comes about through many pairings in writing and speech. Next, the free association task, similar to that used in CITE @Nelson2004 and @SWOW_FIX_THIS was described to the participants as listing the "first word that pops into mind". The participants were then given three example free association cues, *lost*, *old*, and *article*. For each cue, participants were asked to write all the words that come to mind. To elicit free association to the moral foundation areas, participants were given the following instructions:
...
All data was screening for inaccurate responses, as well as missing data, as described in the participant section. Two participants were missing data on the political orientation scale after excluding participants with more than five percent missing data, and this data was excluded pairwise. The MFQ data was screened for multivariate outliers with Mahalanobis distance as described in @Tabachnick2012 CITE, and fourteen outliers were found using $\chi^2_{p < .001}$(`r ncol(finalsample[ , -c(1:6,22,23)])`) = `r apa_num(cutoff1)`. These participants were excluded from further analyses, representing `r nrow(noout1)` final participants. The final MFQ data was screened for assumptions of normality, linearity, homogeneity and homoscedasticity, and these were found to be satisfactory. 

The sum of each moral foundation area was calculated in order to determine which words were linked to their respective moral foundation. The average scores were: harm (*M* = `r printnum(mean(noout1$harmMFQ))`, *SD* = `r printnum(sd(noout1$harmMFQ))`), fairness  (*M* = `r printnum(mean(noout1$fairMFQ))`, *SD* = `r printnum(sd(noout1$fairMFQ))`), ingroup (*M* = `r printnum(mean(noout1$ingroupMFQ))`, *SD* = `r printnum(sd(noout1$ingroupMFQ))`), authority  (*M* = `r printnum(mean(noout1$authorityMFQ))`, *SD* = `r printnum(sd(noout1$authorityMFQ))`), and purity (*M* = `r printnum(mean(noout1$purityMFQ))`, *SD* = `r printnum(sd(noout1$purityMFQ))`). Participants free association responses were processed using the *tm* [CITE] and *corpus* [CITE] libraries after spell checking. Each set of answers was cleaned for punctuation, English stop words (e.g., *the*, *an*, *of*) were removed, and each word was stemmed using the English library in *tm*. We did not combine related words in this section (i.e., *injure* and *injury*, which have different stems *injur* and *injuri*) to allow for maximum coverage of different word forms present in the dictionary. Additionally, with the use of automated stemmers like that present in the *tm* library, leaving both word forms in the dictionary would capture more of the concepts present in future analyses with a different corpus without the requirement on the experimenter to manually recode all word forms. Frequency counts of the stemmed words were tabulated and only words mentioned with at least one percent frequency were used in the subsequent analyses. The complete set of word frequencies for each foundation can be found in our supplemental materials. 
...
write.csv(harm_freq, "exp3_data/study3.1_harm_freq.csv", row.names = F)
...
write.csv(fair_freq, "exp3_data/study3.1_fair_freq.csv", row.names = F)
...
write.csv(ingroup_freq, "exp3_data/study3.1_ingroup_freq.csv", row.names = F)
...
write.csv(authority_freq, "exp3_data/study3.1_authority_freq.csv", row.names = F)
...
write.csv(purity_freq, "exp3_data/study3.1_purity_freq.csv", row.names = F)
...
sample1 <- read.csv("exp3_data/study3.2_fixed_sample 1.csv", stringsAsFactors = F)
sample2 <- read.csv("exp3_data/study3.2_fixed_sample 2.csv", stringsAsFactors = F)
...
write.csv(finalsample, 'exp3_data/study3.2_finalsample.csv', row.names = FALSE)
newMFD <- import('exp3_data/study3.2_finalsample_newMFD.csv')
...
Participant data was first spell checked and screening for inaccurate responses. Participants who did not write more than fifty words in response to a given prompt were excluded (*n* = `r nrow(finalsample) - nrow(finalwords)`). One missing datapoint was estimated using the *mice* library from *R* [@CITE] for a missing MFQ question, and all other missing data was present in the demographics sections, which were not filled in. The MFQ data were screened for outliers using Mahalanobis distance, and 25 outliers were found at $\chi^2_{p<.001}$(`r ncol(allcolumns[ , -c(1,32:42)])`) = `r printnum(cutoff2)`. These data were excluded leading to a final sample size of `r nrow(noout2)`. Data were screened for assumptions described in Study 3.1 and were found to be satisfactory. 
...
write.csv(overall_freq, "exp3_data/study3.2_freq.csv", row.names = F)
...
#write.csv(harm_words_reduce_exp2, "exp3_data/words/harm_exp2.csv", row.names = F)
#write.csv(fair_words_reduce_exp2, "exp3_data/words/fair_exp2.csv", row.names = F)
#write.csv(authority_words_reduce_exp2, "exp3_data/words/authority_exp2.csv", row.names = F)
#write.csv(purity_words_reduce_exp2, "exp3_data/words/purity_exp2.csv", row.names = F)
#write.csv(ingroup_words_reduce_exp2, "exp3_data/words/ingroup_exp2.csv", row.names = F)
```

In the first study, only free association responses were collected, but in this study, full writing samples were collected. Therefore, we expected many of the words listed to be part of creating a cohesive discourse, rather than only related to the moral foundation targeted. To find only the most related words, the correlation between word frequency and moral foundation was calculated and words with correlations greater than two standard deviations outside the mean were selected for the dictionary analysis in Study 3.3.  The sum of each moral foundation area was calculated in order to determine which words were linked to their respective moral foundation. The average scores were: harm (*M* = `r printnum(mean(noout2$harmMFQ))`, *SD* = `r printnum(sd(noout2$harmMFQ))`), fairness  (*M* = `r printnum(mean(noout2$fairMFQ))`, *SD* = `r printnum(sd(noout2$fairMFQ))`), ingroup (*M* = `r printnum(mean(noout2$ingroupMFQ))`, *SD* = `r printnum(sd(noout2$ingroupMFQ))`), authority  (*M* = `r printnum(mean(noout2$authorityMFQ))`, *SD* = `r printnum(sd(noout2$authorityMFQ))`), and purity (*M* = `r printnum(mean(noout2$purityMFQ))`, *SD* = `r printnum(sd(noout2$purityMFQ))`). 

```{r compare-ratings-3.2, include = FALSE}
authority_erin <- read.csv("exp3_data/words/authority_study3.2 erin.csv", stringsAsFactors = F)
authority_will <- read.csv("exp3_data/words/authority_study3.2 will.csv", stringsAsFactors = F)
...
fair_erin <- read.csv("exp3_data/words/fair_study3.2 erin.csv", stringsAsFactors = F)
fair_will <- read.csv("exp3_data/words/fair_study3.2 will.csv", stringsAsFactors = F)
...
purity_erin <- read.csv("exp3_data/words/purity_study3.2 erin.csv", stringsAsFactors = F)
purity_will <- read.csv("exp3_data/words/purity_study3.2 will.csv", stringsAsFactors = F)
...
ingroup_erin <- read.csv("exp3_data/words/ingroup_study3.2 erin.csv", stringsAsFactors = F)
ingroup_will <- read.csv("exp3_data/words/ingroup_study3.2 will.csv", stringsAsFactors = F)
...
harm_erin <- read.csv("exp3_data/words/harm_study3.2 erin.csv", stringsAsFactors = F)
harm_will <- read.csv("exp3_data/words/harm_study3.2 will.csv", stringsAsFactors = F)
...
##original mfd words
original_mfd <- read.csv("exp3_data/original_mfd.csv", stringsAsFactors = F)
